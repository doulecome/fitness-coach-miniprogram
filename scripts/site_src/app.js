...
(function () {
  ...
  function vHome() {
    ...
    var resumeCard = (rs && rs.seq && rs.seq.length && rs.done > 0 && Date.now() - rs.ts < 12 * 3600e3)
      ...
      : '';
    // 今日餐单入口（首页直达，解决"找不到餐单"；无方案自动按默认生成）
    var mealTeaser = '';
    if (!S.dietFormOpen) {
      ensureDiet();
      var th = todayMeals();
      mealTeaser = '<div class="card" data-a="tab" data-v="diet" style="cursor:pointer">' +
        '<div style="display:flex;align-items:center;margin-bottom:7px"><b style="font-size:13.5px">🍱 今日餐单</b>' +
        '<span style="margin-left:auto;font-size:11px;color:var(--sub)">' + th.dayK + ' 千卡/天 · 查看 ▸</span></div>' +
        '<div style="display:flex;flex-wrap:wrap;gap:6px">' +
        ['🌅 ' + th.bf.n, '☀️ ' + th.lunch.n, '🌙 ' + th.dinner.n, '🍪 ' + th.snack.n].map(function (mn) {
