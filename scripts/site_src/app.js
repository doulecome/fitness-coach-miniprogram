...
(function () {
  ...
  function analyzeHealth(h) {
    ...
    if (glu >= 6.1) tips.push({ i: '🩸', t: '空腹血糖偏高（' + glu + ' mmol/L）', d: '建议复查糖代谢；训练加每周 2~3 次有氧，主食换低 GI（糙米/燕麦），少含糖饮料。' });
  ...
  function healthCard() {
    ...
    if (!S.health) {
      return head + '<div style="font-size:11.5px;color:#7a838e;line-height:1.7;margin:4px 0 10px">填入身高体重、血压心率、尿酸血糖等指标（或直接粘贴体检报告文字自动提取），本地生成专属的训练强度与饮食方向建议。数据只保存在这台设备，不上传、不进代码。</div>' +
