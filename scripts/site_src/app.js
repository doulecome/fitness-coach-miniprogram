...
(function () {
  ...
  var S = {
    ...
    diet: loadK('fit_diet', null), dietDraft: { gender: '男', age: 28, height: 175, weight: 70, activity: '中度', goal: '减脂' }, dietFormOpen: false, dietAuto: false,
  ...
  function todayMeals() {
    ...
    if (S.dayMeal && S.dayMeal.date === today && S.dayMeal.goal === d.goal && S.dayMeal.pantry === pk) {
      /* 当日餐单持久化（date+goal+常备食材三重签名）：三者任一变化即按新条件重配 */
      ...
      S.dayMeal = { date: today, goal: d.goal, pantry: pk, bf: bf.n, lunch: lunch.n, dinner: dinner.n, snack: snack.n };
  ...
  /* ============ 训练档案导出/备份（#15） ============ */
  function exportData() {
    var payload = { v: 1, exportedAt: new Date().toISOString(), best: S.best, records: S.records, plan: S.plan, diet: S.diet, dietPair: S.dietPair, pantry: S.pantry, wGoal: S.wGoal, fitness: S.fitness, rpeLog: S.rpeLog, dietLog: S.dietLog };
    try {
  ...
  app.addEventListener('click', function (e) {
    ...
    if (a === 'foodQuick') {
      ...
      if (dq) { addFoodItem(el.dataset.meal, dq.n, dq.kcal); toast('✅ 已记入' + mealLabel(el.dataset.meal) + '：' + dq.n + '（' + dq.kcal + ' 千卡）'); renderView(); }
      return;
    }
    if (a === 'foodPick') {
    ...
    if (a === 'saveDiet') { S.diet = Object.assign({}, S.dietDraft, { training: false }); S.dayMeal = null; S.dietFormOpen = false; S.dietAuto = false; saveK('fit_diet', S.diet); saveK('fit_daymeal', null); renderView(); toast('✅ 方案已生成，已按你的资料重配今日餐单'); return; }
